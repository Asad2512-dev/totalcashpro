@php
    ...
    $wizardConfig = [
        'saveUrl' => route('business-admin.cash-up.store', absolute: false),
        'deductionsUrl' => route('business-admin.cash-up.deductions', absolute: false),
        'csrf' => csrf_token(),
        'initialDate' => $date,
        'initialShift' => $shift,
        'initialStep' => $step,
...
<x-layouts.business-admin title="Cash Up" active="cash-up">
            ...
            <div class="border-b border-gray-200 px-5 pb-4 pt-5 dark:border-gray-700 sm:px-6">
                <div class="mb-4 flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <a href="{{ route('business-admin.cash-up', ['date' => $date, 'shift' => $shift, 'view' => 'cashup', 'step' => $step]) }}" @class(['rounded-full px-4 py-2 text-sm font-semibold transition', 'bg-primary-600 text-white' => $viewTab === 'cashup', 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-200' => $viewTab !== 'cashup'])>Cash Up</a>
                        <a href="{{ route('business-admin.cash-up', ['date' => $date, 'shift' => $shift, 'view' => 'deductions']) }}" @class(['rounded-full px-4 py-2 text-sm font-semibold transition', 'bg-primary-600 text-white' => $viewTab === 'deductions', 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-200' => $viewTab !== 'deductions'])>Platform Deduction</a>
                        <a href="{{ route('business-admin.cash-history') }}" class="ml-1 text-sm font-semibold text-primary-700 hover:underline">History →</a>
                    </div>
                </div>
            ...
            <div class="px-5 py-5 sm:px-6">
                ...
                @if ($viewTab === 'cashup')
                    ...
                    <div class="cashup-actions mt-6">
                        <div>
                            <a
                                href="{{ route('business-admin.cash-up', ['date' => $date, 'shift' => $shift, 'view' => 'cashup', 'step' => max(0, $step - 1)]) }}"
                        ...
                        <div class="text-right">
                            <a
                                href="{{ route('business-admin.cash-up', ['date' => $date, 'shift' => $shift, 'view' => 'cashup', 'step' => min(4, $step + 1)]) }}"
